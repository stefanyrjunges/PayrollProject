...
public class SubmissionManager {
    ...
    public HoursSubmissionData loadStatus() {
        String query = "SELECT wf.employee_id, CONCAT(e.fname, ' ', e.lname) AS fullName, "
                ...
                + "WHERE wf.status = 'pending' AND weekNumber = WEEK(CURDATE(), 1)"
                + "LIMIT 1";

        try (Connection con = DatabaseManager.getConnection(); PreparedStatement psTotal = con.prepareStatement(query); ResultSet rsTotal = psTotal.executeQuery()) {
            ...
            if (rsTotal.next()) {
                ...
                if (rsTotal.getDouble("monday") > 0) {
                    workedDays.append("Monday ");
                }
                if (rsTotal.getDouble("tuesday") > 0) {
                    workedDays.append("Tuesday ");
                }
                if (rsTotal.getDouble("wednesday") > 0) {
                    workedDays.append("Wednesday ");
                }
                if (rsTotal.getDouble("thursday") > 0) {
                    workedDays.append("Thursday ");
                }
                if (rsTotal.getDouble("friday") > 0) {
                    workedDays.append("Friday ");
                }
                if (rsTotal.getDouble("saturday") > 0) {
                    workedDays.append("Saturday ");
                }
                if (rsTotal.getDouble("sunday") > 0) {
                    workedDays.append("Sunday ");
                }
    ...
    public void approveHours(int employeeId) {
        ...
        String query = "UPDATE weekly_finance SET status = 'approved' WHERE employee_id = ? AND weekNumber = WEEK(CURDATE(), 1)";
    ...
    public void rejectHours(int employeeId, String rejectionReason) {

        String query = "UPDATE weekly_finance SET status = 'rejected', rejection_reason = ? WHERE employee_id = ? AND weekNumber = WEEK(CURDATE(), 1)";
